...
class System(pl.LightningModule):
    ...
    def __init__(
        ...
        self.n_splitter_bits, self.n_combiner_bits = n_splitter_bits, n_combiner_bits
    ...
    def common_step(self, batch, batch_nb, train=True):
        ...
        # Pre-process
        inputs_pp = preprocess(inputs, n_splitter=self.model.n_splitter, n_bits=self.n_splitter_bits)
        # Model
        est_targets = self(inputs_pp)
        # Post-process
        est_targets_pp = postprocess(est_targets, n_combiner=self.model.n_combiner, n_bits=self.n_combiner_bits)
        # Loss
        loss = self.loss_func(est_targets_pp, targets)
        return loss
